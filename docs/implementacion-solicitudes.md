# Implementación de Solicitudes (Manager)

Este documento describe cómo implementar, usar y extender la Gestión de Solicitudes en el frontend Manager.

## Resumen
- Servicio central: `src/features/solicitudes/services/solicitudesEdicionService.ts`.
- Tipos: `src/types/solicitudesEdicion.ts`.
- UI del jugador: `src/features/jugadores/components/PlayerSolicitudesEdicion.tsx`.
...
- Aprobar/Rechazar:
  - Crear: aprueba la contraparte. Admin global siempre puede.
...
## UI: PlayerSolicitudesEdicion.tsx
- Filtra solicitudes relacionadas al jugador actual (crear/eliminar/editar).
- Resuelve `contratoId → equipoId` para permisos/etiquetas.
- Muestra detalles y botones según permisos.

## Extender
- Agregar nuevos tipos en `src/types/solicitudesEdicion.ts`.
- Consumir desde el servicio central; evitar adaptadores nuevos.
- Para UI, extender filtros y bloques de detalle.

## Pruebas manuales sugeridas
1) Crear `jugador-equipo-crear` desde equipo y desde jugador → aprobar con la contraparte.
2) `jugador-equipo-eliminar` y `jugador-equipo-editar` → aprobar desde Equipo/Jugador.
3) Verificar cancelaciones: crear vs eliminar/editar.
