// API client for manager panel (gestión-manager)
...
// Basic entities used by manager
export interface Equipo { id: string; nombre: string }
...
export const api = {
  ...
  listEquipoPartidos: (equipoId: string) => request<Partido[]>(`/partidos?equipoId=${equipoId}`),
  crearAmistoso: (payload: Partial<Partido>) => request<Partido>(`/partidos`, { method: 'POST', body: payload }),
  registrarEstadisticasPartido: (partidoId: string, stats: any) => request<any>(`/partidos/${partidoId}/estadisticas`, { method: 'POST', body: stats }),
  listJugadores: (equipoId?: string) => request<Jugador[]>(`/jugadores${equipoId ? `?equipoId=${equipoId}` : ''}`),
  ...
  listSolicitudes: () => request<SolicitudEdicion[]>(`/solicitud-edicion?creadoPor=me`),
};
export default api;
