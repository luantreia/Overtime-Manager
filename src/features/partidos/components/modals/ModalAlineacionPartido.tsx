...
export const ModalAlineacionPartido = ({
  ...
  const handleAgregarJugador = async (equipo: 'local' | 'visitante') => {
    try {
      ...
      if (err instanceof InvalidObjectIdError) {
        addToast({ type: 'error', title: 'ID inválido', message: 'El identificador del partido o equipo no es válido.' });
      } else {
  ...
  const handleQuitarJugador = async (jugadorId: string) => {
    ...
    try {
      ...
      if (err instanceof InvalidObjectIdError) {
        addToast({ type: 'error', title: 'ID inválido', message: 'El identificador del partido o jugador no es válido.' });
      } else {
