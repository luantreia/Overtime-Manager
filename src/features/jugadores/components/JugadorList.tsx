            ...
            {jugadores.map((jugador) => (
              <tr key={jugador.id} className="hover:bg-slate-50/60">
                ...
                <td className="px-6 py-4">
                  {(() => {
                    const key = jugador.estado ?? 'unknown';
                    const classes = estadoColorMap[key] ?? 'bg-slate-100 text-slate-700';
                    return (
      ...
      <section className="rounded-2xl border border-amber-200 bg-amber-50/70 p-6 shadow-sm">
        ...
        <ul className="space-y-4">
          {solicitudesPendientes.map((solicitud) => {
            ...
            const showCancel = (solicitud.origen === 'jugador' && (isSolicitante || isJugadorAdmin || isAdminGlobal)) || (solicitud.origen === 'equipo' && (isEquipoAdmin || isAdminGlobal));
            const showAccept = (solicitud.origen === 'jugador' && (isEquipoAdmin || isAdminGlobal)) || (solicitud.origen === 'equipo' && (isJugadorAdmin || isAdminGlobal));
            ...
            return (
              <li key={solicitud.id} className="flex flex-wrap items-center gap-3 rounded-xl bg-white px-4 py-3 text-sm shadow-sm">
                ...
                <div className="flex gap-2">
                  {showCancel && onRechazarSolicitud ? (
                    <button
                      type="button"
